...
class Main{
    ...
    public static void main(String[] args){  
        ...
        int [] anubhav = new int[people];
        
        for (int i = 0; i<people ; i++) anubhav[i] = i+1;
        
        int singh = sc.nextInt();
        
        int thakur = sc.nextInt();
        
        System.out.println(cheating(anubhav,0,people-1,0,singh,thakur));
        
    }
    
    public static List< List<Integer>> plag = new ArrayList<>();
    ...
    public static int cheating( int[] arr, int singh, int anu, int sum, int singh1, int thakur1){
            if (singh>anu){
            if (sum %2 == 1){
                return 1;
            }
        ...
        int plag = 0;
        for ( int i = singh; i<=anu; i++){
            swap(arr,i,singh);
            if (singh>= singh && singh<=thakur1){
                plag += cheating(arr, singh+1, anu , sum + arr[singh],singh1,thakur1);
            }else{
                plag+= cheating(arr,singh+1, anu, sum,singh1,thakur1);
            }
            swap(arr,i,singh);
        }
        return plag;
